...
task("jshint", "run jshint",
    require("./config/tasks/jshint")(config));
task("jsbeautifier", "run jsbeautifier",
    require("./config/tasks/jsbeautifier")(config));

task("default",
    task.series(task("jsbeautifier"), task("jshint")));


task("config", "compile config to build directory",
    require("./config/tasks/config")(config));
task("js", "compile js into one file",
    task.series(task("config"), require("./config/tasks/comn")(config)));

task("css", "compile css into one file",
    require("./config/tasks/comn_css")(config));
...
task("clean", "clean build files",
    require("./config/tasks/clean")(config));
...
task("copy", "copys app fonts to build dir",
    task("copy_imgs"));


task("serve", "serve build directory",
...
task("minify", "minify built app", task.parallel(
    task("uglify"),
    task("minify_css"),
    task("minify_html"),
    task("minify_json")));
...
if (config.env !== "production" && config.env !== "staging") {
    task("build", "builds app in " + config.env,
        task.parallel(task("js"), task("css"), task("ejs"), task("locale")));
} else {
    task("build", "builds app in " + config.env, task.series(
        task.parallel(
            task("js"), task("css"), task("ejs"), task("copy"), task("locale")),
        task("minify")
    ));
}
...
task("reload", reload);
task("js_reload", "builds js and calls for a reload",
    task.series(task("js"), task("reload")));
...
task("run", "builds app and starts watching files",
    task.series(task("build"), task("watch"), task("serve")));
